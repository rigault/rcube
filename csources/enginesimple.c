...
static inline int forwardSectorOptimize (const Pp *pOr, const Pp *pDest, int nIsoc, const Pp *isoList, int isoLen, Pp *optIsoc) {
   ...
   if (par.jFactor == 0 || nIsoc < LIMIT) { // LIMIT SOULD BE > 0 
      ...
      //orthoFindInterPoint(pOr->lat, pOr->lon, pDest->lat, pDest->lon, dist, &focalLat, &focalLon);
...
static int buildNextIsochrone (const Pp *pOr, const Pp *pDest, const Pp *isoList, int isoLen,
   ...
   *biggestOrthoVmc = 0;
   ...
   for (int k = 0; k < isoLen; k++) {
      ...
      else efficiency = isDayLight (&localTm, t, isoPt->lat, isoPt->lon) ? par.dayEfficiency : par.nightEfficiency;
...
/*! find index of point with ptId id in isochrone i*/
static int findPtIndexById (int id, int i, int lIsoc) {
    Pp *iso = &isocArray [i * MAX_SIZE_ISOC];  // Access isoc i
    for (int k = 0; k < lIsoc; k++) {
        if (iso[k].id == id) return k;
    }
    fprintf (stderr, "In findPtIndexById, Error id not found: %d, Isoc No:%d, Isoc Len: %d\n", id, i, lIsoc);
    return -1;
}
...
bool storeRoute (SailRoute *route, const Pp *pOr, const Pp *pDest) {
   ...
   for (int i = route->n - 3; i >= 0; i--) {
      iFather = findPtIndexById (pt.father, i, isoDesc[i].size);
      //printf ("ISOC: %d, ID: %d, FATHER: %d, TO_INDEX_WP: %d\n", i, pt.id, pt.father, pt.toIndexWp); 
      if (iFather == -1) {
         fprintf (stdout, "In storeRoute: ERROR findPtIndexById at isoc: %d\n", i);
         return false;
      }
...
*/
static inline bool goalP (const Pp *pA, const Pp *pDest, double t, double dt, double *timeTo,
   ...
   *motor = useMotor;
   ...
   struct tm tm0 = gribDateToTm(zone.dataDate[0], zone.dataTime[0] / 100); 
   if (par.dayEfficiency == par.nightEfficiency) efficiency = par.dayEfficiency;
   else efficiency = isDayLight(&tm0, t, pA->lat, pA->lon) ? par.dayEfficiency : par.nightEfficiency;
...
static inline bool goal (Pp *pDest, Pp *isoList, int len, double t, double dt, double *lastStepDuration, bool *motor, int *amure) {
   ...
   for (int k = 0; k < len; k++) {
      ...
      if (goalP (curr, pDest, t, dt, &time, &distance, &locMotor, &locAmure, &sail)) {
...
static int routing (Pp *pOr, Pp *pDest, int toIndexWp, double t, double dt, double *lastStepDuration) {
   ...
   if (goalP (pOr, pDest, t, dt, &timeToReach, &distance, &motor, &amure, &sail)) {
      pDest->father = pOr->id;
      pDest->motor = motor;
      pDest->amure = amure;
      pDest->sail = sail;
...
static void checkArrival (SailRoute *route, Pp *pDest) {
   ...
   int index;
   if (route->destinationReached) index = findPtIndexById (pDest->father, lastI, size);
   else index = findPtIndexById (pDest->id, lastI, size);
